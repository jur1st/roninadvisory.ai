/**
 * rag-web-damage-control — Tool-call interception and rule enforcement.
 *
 * Copied from examples/pi-vs-claude-code/extensions/damage-control.ts in the
 * Pi Agent workshop. Two deviations from verbatim:
 *   1. The `applyExtensionDefaults` import and its call inside session_start
 *      are omitted, so loading this safety extension does not override the
 *      operator's theme-cycler on boot.
 *   2. All three `ctx.ui.setStatus(...)` calls use the two-argument keyed form
 *      with slot "damage-control". The reference single-argument form does not
 *      persist in the status footer (confirmed by cross-referencing every
 *      setStatus call in ai_docs/pi_official/examples-extensions/). Candidate
 *      for upstreaming.
 * Rule-engine and tool_call logic are unchanged.
 *
 * Layer in the three-layer safety contract (see docs/agent/conventions.md):
 * this extension IS layer 2 for bash-enabled interactive turns. Subprocess runs
 * that pass --no-extensions bypass it — those are covered by the orchestrator
 * checkpoint (layer 3) and the per-agent tool allowlist (layer 1).
 *
 * Rules file: .pi/damage-control-rules.yaml (project) or ~/.pi/damage-control-rules.yaml (global).
 * Project rules take precedence. Extending the rule set is a YAML edit, not a code change —
 * the TypeScript body is intended to stay upstream-compatible with the reference project.
 */
