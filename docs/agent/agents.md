...
**Single-call-site doctrine.** `capture-screenshot.sh` is the only sanctioned surface for browser captures. The agent body explicitly prohibits authoring ad-hoc `.js` scripts alongside the wrapper. This prohibition is not pedantry — a review that routes around the wrapper leaves behind a script that drifts from the wrapper's feature set (viewport override, `--scale`, correct `data-theme` pinning). Two capture paths produce two different "truths" about how the site looks; the wrapper is the single source of "how we see the site." If the wrapper is insufficient for a new capture need, the correct action is to extend `capture-screenshot.sh` (and update the agent's documented flags), not to author a sibling script. The plan-04 Step 10 review surfaced a real case where a viewport gap forced inline authoring of `_review-capture.js` — that case is the warrant for the explicit rule now in the agent body. The flags that closed the gap (`--viewport`, `--scale`, deterministic `data-theme` pinning) are why the escape hatch was removed.
